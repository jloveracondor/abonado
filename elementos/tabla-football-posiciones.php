<div class="container-fluid container-ab my-5" id="ab-posiciones">
	<div class="row">
		<div class="col">
			<h1 class="ab-title">Posiciones por Equipo</h1>
		</div>
	</div>

	<div class="row my-3">
		<div class="col-2">
			<select class="form-control" id="exampleFormControlSelect1">
        <option>FIFA 2022</option>
				<option>UEFA</option>
				<option>LA LIGA</option>
			</select>
		</div>
		<div class="col-2">
			<select class="form-control" id="exampleFormControlSelect1">
        <option>SURAMÉRICA</option>
				<option>ÁFRICA</option>
				<option>ASIA</option>
				<option>EUROPA</option>
				<option>N / CENTRO AMÉRICA Y CARIBE</option>
				<option>OCEANÍA</option>
			</select>
		</div>
    <div class="col-2">
			<select class="form-control" id="exampleFormControlSelect1">
				<option>EQUIPOS</option>
				<option>JUGADORES</option>
			</select>
		</div>
		<!-- <div class="col-8 align-middle">
      
      La siguiente es por si deseas colocar el tab list a la derecha de los seleccionadores de arriba
			<nav>
				<div class="nav nav-tabs ab-tabs" id="nav-tab" role="tablist">
					<a class="nav-item nav-link active" id="nav-regular-tab" data-toggle="tab" href="#nav-regular" role="tab" aria-controls="nav-regular" aria-selected="true">Regular</a>
					<a class="nav-item nav-link" id="nav-segunda-tab" data-toggle="tab" href="#nav-segunda" role="tab" aria-controls="nav-segunda" aria-selected="false">Segunda Vuelta</a>
				</div>
      </nav>
      
		</div> -->
	</div>
	<!-- Tabs-->
	<div class="tab-content" id="nav-tabContent">
		<div class="tab-pane fade show active" id="nav-regular" role="tabpanel" aria-labelledby="nav-regular-tab">
		<div class="row ">
		<div class="col"> 
			<div class="col rounded border table-responsive">
				<p class="ab-title-liga">FIFA World Cup 2022 Qualifiers - COMMEBOL <img src="/abonado/assent/img/fifa2022.png" style="max-width: 42px;"alt=""></p>
				<table class="table table-hover ab-table">
					<thead>
						<tr class="ab-titulos-table text-center">
							<th scope="col"></th>
							<th scope="col">EQUIPO</th>
							<th scope="col"></th>
							<th scope="col">PJ</th>
							<th scope="col"></th>
							<th scope="col">PG</th>
							<th scope="col"></th>
							<th scope="col">PE</th>
							<th scope="col"></th>
              <th scope="col">PP</th>
              <th></th>
              <th>GF</th>
              <th></th>
							<th class="ab-average" scope="col" data-toggle="tooltip" data-placement="top" title="Goles en Contra"> GC </th>
	
							<th scope="col"> </th>
							<th scope="col"> DG </th>
							<th scope="col"></th>
							<th scope="col"> PTS </th>
							<th scope="col"></th>
							
							
						</tr>
					</thead>
					<tbody class="text-center">
            <!-- start tr-->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						<tr>
              <td>1</td>
							<td><img src="/abonado/assent/img/banderas-grandes/ARG.png" alt="">Argentina</td>
							<td></td>  <!-- Aquí 2da-->
							<td>5</td>
							<td></td>
							<td>3</td>
							<td></td>
              <td>4</td>
							<td></td>
							<td>0</td>
							<td></td>
							<td>2</td>
              <td></td>
							<td class="ab-average-fondo">1</td>
							<td></td>
              <td>1</td>
              <td></td>
              <td>3</td>
              <td></td>
            </tr>
						<!-- end tr -->
						
					</tbody>
				</table>

			</div>
		</div>
	</div>

		</div>

	</div>
	
</div>